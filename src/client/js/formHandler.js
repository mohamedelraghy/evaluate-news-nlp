  ...
  if (checkForUrl(getUrl)) {
    ...
    postData(`http://localhost:8081/`, {
      ...
      document.getElementById(
        "text"
      ).innerHTML = `SubjectText: ${apiData.text}`;
      document.getElementById(
        "agreement"
      ).innerHTML = `Agreement: ${apiData.agreement}`;
      document.getElementById(
        "confidence"
      ).innerHTML = `Confidence: ${apiData.confidence}`;
      document.getElementById(
        "subjectivity"
      ).innerHTML = `Subjectivity: ${apiData.subjectivity}`;
